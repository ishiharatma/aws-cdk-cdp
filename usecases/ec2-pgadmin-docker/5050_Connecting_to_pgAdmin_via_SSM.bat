...
SET SERVER_NAME=pgAdmin4
SET AWS_ACCOUNT_NAME=xxxx

REM ****************************
REM Please specify the profile name set in .aws\credentials
SET AWS_PROFILE=foo-bar

REM ****************************
REM Please specify a non-conflicting port in the terminal you are running
SET LOCAL_PORT_NO=5050

REM PgAdmin Listen Port
SET SERVER_PORT_NO=5050

echo.
echo Connecting to pgAdmin via SSM
echo.
echo ^<!!!!! check !!!!!^>
...
echo.
echo After connecting, please connect to the following URL in your browser.
echo http://localhost:%LOCAL_PORT_NO%/
REM If you would like to display your connection information, please enter it below.
echo LoginUser: N/A
echo LoginPassword: N/A
echo.
